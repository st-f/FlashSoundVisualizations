package
{
	...
	public class Away3DCubes extends Sprite
	{
		...
		protected static const CUBESIZE:int=50;
		...
		public function Away3DCubes()
		{
			...
			stats.y=5;
			this.addChild(stats);
			myMaterial=new ColorMaterial(0xFFCC00);
		...
		protected function setupLights():void
		{
			...
			directionalLight.position=new Vector3D(0, 100, 0);
			directionalLight.ambient=2;
			directionalLight.diffuse=0;
			scene.addChild(directionalLight);
			pointLight=new PointLight();
			pointLight.position=new Vector3D(0, 100, 0);
		...
		{
			if (_soundBytes.length < FREQLINES)
				return;
			var dest:Number;
			total=0;
			...
			if (move == false)
			{
				cameraController.panAngle=total / FREQLINES * 180 * 10;
			}
			//trace("tween: "+dest);
		}
