package 
{
	...
	public class Away3DGlassCubes extends Sprite
	{
		...
		protected var pointLight2:PointLight;		
		...
		protected function createMaterial():void
		{
			...
			lightPicker1 = new StaticLightPicker([pointLight, pointLight2, directionalLight]);
		...
		protected function setupLights():void
		{
			...
			directionalLight.position = new Vector3D(2000, 2500, 2000);
			directionalLight.lookAt(new Vector3D());
			directionalLight.ambient = 2.7;
			directionalLight.diffuse = .5;
			directionalLight.color = 0xFFFF00;
			scene.addChild(directionalLight);
			pointLight = new PointLight();
			pointLight.position = new Vector3D(2000, -2000, -2000);
			pointLight.lookAt(new Vector3D());
			pointLight.ambient = .05;
			pointLight.diffuse = .5;
			pointLight.color = 0x00FF00;
			scene.addChild(pointLight);
			pointLight2 = new PointLight();
			pointLight2.position = new Vector3D(-2000, 0, -2000);
			pointLight2.lookAt(new Vector3D());
			pointLight2.ambient = .05;
			pointLight2.diffuse = .5;
			pointLight2.color = 0x00FF00;
		...
		protected function animate(event:TimerEvent = null):void
		{
			...
			TweenLite.to(cameraController, animationSpeed/100,{distance:0});
			TweenLite.to(cameraController, animationSpeed/10,{tiltAngle:total/FREQLINES * 100});			
			
			/*var tint:uint = calculateTint(100-total/20,0x00FF00);
			directionalLight.color = tint;
			pointLight.color = tint;
			pointLight2.color = tint;*/
			/*TweenLite.to(directionalLight, animationSpeed/10,{color:tint});
			TweenLite.to(pointLight, animationSpeed/10,{color:tint});
			TweenLite.to(pointLight2, animationSpeed/10,{color:tint});*/
			
			valueRows.unshift(valueLine);
			animateRowsAndColumns();
		}
		
		protected function calculateTint(percent:Number,color:Number):Number 
		{
			var n:int = 0;
			if(n<0 || n>100 || color<0 || color>0xffffff)
			{
				return Number.NaN;
			}
			else 
			{
				...
				r += Math.round((0xff-r)*(100-percent)/100);
				g += Math.round((0xff-g)*(100-percent)/100);
				b += Math.round((0xff-b)*(100-percent)/100);
				trace(r.toString(16)+" : "+g.toString(16)+" : "+b.toString(16));
				return (r << 16) | (g << 8) | b;
			}
		}
